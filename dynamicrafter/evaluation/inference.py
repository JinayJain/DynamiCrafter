...
from dynamicrafter.models.samplers.ddim_multiplecond import (
...
def load_data_prompts(data_dir, video_size=(256, 256), video_frames=16, interp=False):
    ...
    for idx in range(n_samples):
        if interp:
            ...
            frame_tensor1 = repeat(
                image_tensor1, "c t h w -> c (repeat t) h w", repeat=video_frames // 2
            )
            frame_tensor2 = repeat(
                image_tensor2, "c t h w -> c (repeat t) h w", repeat=video_frames // 2
            )
...
def save_results_seperate(prompt, samples, filename, fakedir, fps=10, loop=False):
    ...
    for idx, video in enumerate(videos):
        ...
        for i in range(n):
            ...
            path = os.path.join(
                savedirs[idx].replace("samples", "samples_separate"),
...
def run_inference(args, gpu_num, gpu_no):
    ...
    print(
        "Prompts testing [rank:%d] %d/%d samples loaded."
    ...
    with torch.no_grad(), torch.cuda.amp.autocast():
        for idx, indice in tqdm(
            enumerate(range(0, len(prompt_list_rank), args.bs)), desc="Sample Batch"
        ):
            ...
            for nn, samples in enumerate(batch_samples):
                ...
                save_results_seperate(
                    prompt, samples, filename, fakedir, fps=8, loop=args.loop
                )
...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="a data dir containing videos and prompts",
    )
    parser.add_argument(
        "--n_samples",
        type=int,
    ...
    parser.add_argument(
        "--bs", type=int, default=1, help="batch size for inference, should be one"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--seed", type=int, default=123, help="seed for seed_everything"
    )
    parser.add_argument(
        "--video_length", type=int, default=16, help="inference video length"
    )
    parser.add_argument(
        "--negative_prompt", action="store_true", default=False, help="negative prompt"
    )
    parser.add_argument(
        "--text_input",
        action="store_true",
        default=False,
        help="input text to I2V model or not",
    )
    parser.add_argument(
        "--multiple_cond_cfg",
        action="store_true",
