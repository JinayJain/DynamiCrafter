...
class Image2Video:
    def __init__(self, result_dir="./tmp/", gpu_num=1, resolution="256_256") -> None:
        ...
        ckpt_path = (
            "checkpoints/dynamicrafter_"
            + resolution.split("_")[1]
            + "_interp_v1/model.ckpt"
        )
    ...
    def get_image(
        ...
        with torch.no_grad(), torch.cuda.amp.autocast():
            ...
            if image2 is not None:
                img_tensor2 = (
                    torch.from_numpy(image2).permute(2, 0, 1).float().to(model.device)
                )
                img_tensor2 = (img_tensor2 / 255.0 - 0.5) * 2
