...
class Image2Video:
    def __init__(self, result_dir="./tmp/", gpu_num=1, resolution="256_256") -> None:
        self.resolution = (
            int(resolution.split("_")[0]),
            int(resolution.split("_")[1]),
        )  # hw
        self.download_model()
        ...
        ckpt_path = (
            "checkpoints/dynamicrafter_" + resolution.split("_")[1] + "_v1/model.ckpt"
        )
    ...
    def get_image(
        ...
        with torch.no_grad(), torch.cuda.amp.autocast():
            ...
            # img cond
            img_tensor = (
                torch.from_numpy(image).permute(2, 0, 1).float().to(model.device)
            )
            ...
            img_tensor_repeat = repeat(
                z, "b c t h w -> b c (repeat t) h w", repeat=frames
            )
        ...
        save_videos(
            batch_samples, self.result_dir, filenames=[prompt_str], fps=self.save_fps
        )
    ...
    def download_model(self):
        REPO_ID = (
            "Doubiiu/DynamiCrafter_" + str(self.resolution[1])
            if self.resolution[1] != 256
            else "Doubiiu/DynamiCrafter"
        )
        filename_list = ["model.ckpt"]
        if not os.path.exists(
            "./checkpoints/dynamicrafter_" + str(self.resolution[1]) + "_v1/"
        ):
            os.makedirs(
                "./checkpoints/dynamicrafter_" + str(self.resolution[1]) + "_v1/"
            )
        for filename in filename_list:
            ...
            if not os.path.exists(local_file):
                hf_hub_download(
                    repo_id=REPO_ID,
                    filename=filename,
                    local_dir="./checkpoints/dynamicrafter_"
